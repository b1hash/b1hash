...
//                    }
    ...
    private void getPayType() {
        CommonHttpUtil.getPayType(new HttpCallback() {
            ...
            public void onSuccess(int code, String msg, String[] info) {
                if (code == 0 && info.length > 0) {
                    mPayList = JSON.parseArray(Arrays.toString(info), CoinPayBean.class);
                }
            }
        });
    }
...
//        }
        String money = mCheckedCoinBean.getMoney();
        CommonHttpUtil.getPayUrl(money,mCheckedCoinBean.getId(), mCheckedCoinBean.getCoin(), coinPayBean.getPaytype(), new HttpCallback() {
