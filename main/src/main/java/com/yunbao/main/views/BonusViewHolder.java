...
public class BonusViewHolder extends AbsViewHolder implements View.OnClickListener {
    ...
    public void setData(List<BonusBean> list, int day, String continueDay) {
        mList = list;
        mDay = day;
        String s = String.format(WordUtil.getString(R.string.bonus_sign_1), continueDay);
        SpannableString spannableString = new SpannableString(s);
        int startIndex = s.indexOf(continueDay);
        if (startIndex >= 0) {
            spannableString.setSpan(new ForegroundColorSpan(ContextCompat.getColor(mContext, R.color.global)), startIndex, startIndex + continueDay.length(),
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        mTotalDay.setText(spannableString);
        String s1 = WordUtil.getString(R.string.bonus_sign_3);
        for (int i = 0; i < 7; i++) {
